...
/*
Plugin Name:KSES
Plugin URI:http://sourceforge.net/projects/kses
Description:Removes certain HTML from comments.
Version:0.2.1
Author:Ulf Harnhammar
Author URI:mailto:metaur(at)users(dot)sourceforge(dot)net
Required Framework: 1.0.0

	Converted to a TextBones plugin by David Carrington.
	Improved as an ajfork plugin by Øivind Hoel

*/

@add_filter('news-posted-comment-content','kses_filter');
@add_filter('add-comment-box','kses_printtags');
@add_filter('template-variables-commentform','kses_addtemplatevars', '50');


#function kses_printtags($output, $hook, $fetchtags="no") {
#$allowed_html = array(
#                 'a' => array('href' => 1,
#                              'title' => 1),
#		'b' => array(),
...
#		'p' => array(),
#	);
#	foreach ($kses_allowed_tags as $tag => $null) {
#		$kses_printtags .= "&lt;$tag&gt;, ";
#		}
#	$output = str_replace("{allowedtags}", $kses_printtags, $output);
#	if ($fetchtags == "yes") { unset($output); $output = $kses_allowed_tags; }
#	return $output;
#}

function kses_addtemplatevars($output, $hook) {
	$output["{allowedtags}"] = "Displays list of html tags that will be allowed through the kses tag-filter.";
	return $output;
}

#function kses_filter($text,$hook) {
#global $allowed_html;
#	$kses_allowed_tags = kses_printtags("no", "no", "yes");
...
# in $attr2
  ...
  foreach ($attrarr as $null => $arreach)
  {
...
###############################################################################
{
  ...
  $allowed = false;
  foreach ($allowed_protocols as $null => $one_protocol)
